...
Following selection, we assume that parasites can go under mutation before infecting hosts. Following the approach by \cite{ashby2015diversity}, we can find the density of parasite with genotype $i$:
$$
