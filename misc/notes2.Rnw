...
Let $A_{ij}$ and $S_{ij}$ represent density of total asexual and sexual population with genotype $ij$.
...
\end{aligned}
$$
where $s$ is the proportion of male offspring produced, $S_0$ is the total density of the sexual population, $g_i$ is the frequency of gamete $i$ before recombination and $P_{ij}$ is the probability of infection. $W_I$ and $W_U$ are modeled in the same way as \cite{lively2010epidemiological}. After recombination and segration, we get the expected number of individuals in the next generation for each genoytpe and ploidy. Then, we get the number of individuals in the next generation from negative binomial distribution. We also assume that an individual enters the population for every genotype stochastically.

Let $\lambda_i$ be the mean number of poisson mean number of exposures for hosts with genotype $ij$ by parasite with haplotype $i$, we can write
$$
P_{ij} = 1 -\exp(-(\lambda_i+\lambda_j)/2).
$$
By doing so, we are assuming that effects of dominance do not exist. Then, we get
$$
\lambda_i = \beta \left(\sum_{p} (S_{ip,i,I} + A_{ip,i,I})\right)/N
$$
To avoid fixation, we assume an individual infected with each genotype enter population stochastically.

Here, density of genotype $ip$ infected with haplotype $i$ -- $S_{ij,i,I}$ and $A_{ij,i,I}$ -- is given by 
$$
S_{ij,i,I} = \frac{\lambda_i(t-1)}{\lambda_{i}(t-1)+\lambda_j(t-1)} S_{ij,I},...
$$
