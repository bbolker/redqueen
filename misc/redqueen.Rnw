...
Here, we show that (1) model parameters are estimable and (2) a simple Red Queen model with population structure can reproduce key summary statistics of observed data from three different snail populations (\fref{smcsumm}).
...
As lack of variation explains low power, sampling sites with large variation in infection prevalence will increase power to detect the positive correlation. %% TODO? show this?
