...
\fref{ivs} may appear to suggest that the model fits are worse overall than what we expected by looking at \fref{smcsumm}.
...
Our results challenge the way the Red Queen Hypothesis for sex is currently studied.
We show that a simple Red Queen model with population structure can reproduce mean level and variation in infection prevalence and frequency of sexual reproduction observed across three different snail populations [REF].
We also show that predicted relationships between infection prevalence and frequency of sexual reproduction can be different from those observed in nature even when the observed summary statistics are matched by the model [REF].
Our approach contrasts with previous studies that have relied on assumed parameters to make inference about a system that goes through the Red Queen dynamics [CITE].
However, discrepancies between the predicted and observed trends suggest that current Red Queen models may not be sufficient to explain effects of parasites on maintanence of sex observed in natural systems.
We find that detecting a positive correlation between infection prevalence and frequency of sexual hosts is difficult.
This finding calls for different methods to detect parasite mediated selection for sex.

A model that does not fit well can carry more information than a model that fits well.
In order for our model to produce similar summary statistics as those observed in the snail-trematode systems, it requires the scale parameter ($c_b$) for cost of sex to be greater than 1 [REF].
Although $c_b$ was initially defined as additional costs and benefits of sex by [CITE], it takes on a different meaning from an inferential point of view.
In the absence of the parameter $c_b$ (i.e, when $c_b$ = 1), the model assumes two fold cost of sex in growth rate.
Then, an estimate of $c_b > 1$ should be interpreted as the amount of compensation required to overcome the two fold cost of sex.
A simpler model without the scaling parameter will not be able to reproduce observed summary statistics.
Here, additional compensation may include other mechanisms that can help maintenance of sex, such as higher genetic diversity, and [TODO; CITE].

The biggest limitation of our model is genetic diversity. 
Here, we assume that infection is deteremined two biallelic loci.
Although exact number loci that determine trematode infecion in snails is not known, it is not likely that a two loci model is sufficient given genetic diversity of snails [CITE].
In addition, high estimate of infection prevalence compared to obseved prevalence [REF] can be explained by limited genetic diversity of the model.
...
Therefore, a model with limited genetic diversity is expected to have higher infection prevalence given amount of sexually reproducing hosts.

While all three studies \citep{dagan2013clonal, mckone2016fine, vergara2014infection} considered similar snail-trematode systems, the snail populations studied by \cite{dagan2013clonal} lives in intrinsically different environments.
Some populations are subject to flash floods, which can disturb habitats and affect selection for sexual reproduction \citep{ben2007temporal}.
Our simple model that does not take any environmental factors into account ends up fitting badly.
Overall, our result indicates that more modeling effort is necessary to understand maintenance of sexual reproduction in nature.

Our power analysis appears to contradict the positive correlation predicted by \cite{lively2001trematode} and findings of many empirical studies that have confirmed the prediction [CITE].
Since parasitism is the only mechanism that drives sexual reproduction in our model, we expected the power to be high.
However. the power analysis predicts almost no power for detecting a positive corrleation in the population studied by \cite{vergara2014infection} and higher power for detecting a negative correlation, contradicting an earlier finding by \cite{vergara2013geographic} that reported a positive correlation between between infection prevalence and male frequency in the same population.
...
In particular, comparing \cite{vergara2013geographic} and \cite{vergara2014infection} shows that the positive correlation in the population is driven by populations that have low infection prevalence and low frequency of sexual hosts (i.e. cannot provide much support for the Red Queen Hypothesis) acting as leverage points.
The leverage effect is even clearer in figure 3 of \cite{kumpulainen2004parasites}.
Since both data sets provided by \cite{vergara2014infection} and \cite{mckone2016fine} only consists of populations with high and low infection prevalence respectively, there is not enough variance to observe the leverage effect.
...
Furthermore, the prediction by \cite{lively2001trematode} was based on expected mean prevalence of infection and frequency of sexual hosts and did not take temporal variation into account.
