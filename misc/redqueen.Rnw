...
On the other hand, empiricists have mostly focused on indirectly tests that stem from the hypothesis.
...
% None of the Red Queen models reviewed by \cite{ashby2015diversity} use statistical tools to connect the model to data.
An exception is work by \cite{lively1992parthenogenesis}.
...
The most striking result from \fref{ivs} is that there is a region in which proportion of infected hosts increases while proportion of sexual hosts increases (most clearly seen from the fits to \cite{mckone2016fine} and \cite{vergara2014infection}). %% TODO: add circles around it??
...
%% We suggest modelers to not limit genetic diversity and empricists to document genetic architecture that determine parasite infection???
