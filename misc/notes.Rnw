...
tframe <- 950:1200
## sex vs asex
plot(infection_res$S.count[tframe], type="l", ylim=c(0,1e4))
lines(infection_res$A.count[tframe], col=2)

## sex vs asex prevalence
with(infection_res,{
    plot(SI.count[tframe]/(S.count[tframe]+SI.count[tframe]), type="l", ylim=c(0,1))
    lines(AI.count[tframe]/(A.count[tframe]+AI.count[tframe]), col=2)
})
