...
Some theoretical studies took a departure from the classical population genetics framework and studied effects of ecological and epidemiological on the Red Queen dynamics.
On one hand, number of studies showed that incorporating ecological and epidemiological details can support sexual reproduction in the host population \citep{galvani2001antigenic, galvani2003maintenance, lively2009maintenance, lively2010epidemiological}.
...
Empiricists have mostly focused on confirming predictions that stem from the Red Queen hypothesis.
Typical among them are local adaptation, time-lagged selection, and association between parasites and host reproductive mode (see \cite{tobler2008expanding} and \cite{vergara2014infection} for reviews).
...
Recently, \cite{auld2016sex} and \cite{slowinski2016coeveolutionary} showed that more direct evidence can be achieved using experimental systems.

While the Red Queen Hypothesis has gained reasponable support both theoretically and empirically, there still remains a gap between theory and data.
...
\citep{lively2001trematode} then formalized the idea with a mathematical model and confirmed the prediction with a snail-trematode system.
Many other emprical works have successfully found the positive correlation \citep{lively2002temporal, kumpulainen2004parasites, vergara2013geographic, mckone2016fine, gibson2016within} but surprisingly such correlation was not observed in a different snail-trematode system \citep{dagan2013clonal}.

Here, we try to bridge the gap between theory and data further.
We extend the model used by \cite{lively2010epidemiological} to include stochasticity and population structure.
Then, we fit it the to data from \cite{dagan2013clonal, mckone2016fine, vergara2014infection} using Approximate Bayesian Computation (ABC) to estimate biologically relavent parameters.
Using estimated parameters obtained from the fits, we assess the model by comparing its fit to data and perform a power analysis to test the prediction that infection prevalence is positively correlated with frequency of sexual reproduction \cite{lively2001trematode}.
