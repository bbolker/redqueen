...
CV of these proportions across populations is calculated by first calculating mean proportions by averaging across generations for each site (population) and then taking the the CV of these mean proportions.
Likewise, CV across generations is calculated by first averaging proportions across space (population) at each generation and then taking the CV.
...
Tolerances of 1.6, 0.8, 0.6 and 0.4 are used for each run.
...
Tolerance of 2.4, 1.2, 0.9 and 0.6 are used for each run to account for higher number of summary statistics being compared.
Tolerances are chosen so that a parameter set will be accepted during the final run only if its each simulated summary statistic deviates from the corresponding observed summary statistic by 0.1 units on average.
