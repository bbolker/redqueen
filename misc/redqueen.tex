...
Why does sex continue to persist despite its cost?
...
For example, Lively predicted frequency of sexual hosts to be positively correlated with prevalence of infection using a mathematical model;
the prediction was confirmed in numerous studies involving snail-trematode systems in New Zealand but the correlation was consistently not detected from snail-trematode systems in Israel.
In this study, we fit a simple host-parasite coevolution model to observational data from those snail-trematode systems and perform power analysis to test how likely it is to observe the predicted correlation.
We discuss ways in which model prediction differs from observation (e.g., high power predicted for the Israel snail system) and provide practical guidance to both modelers and empiricists.
...
First, hosts and parasites must coevolve [CITE].
...
Israel population is of particular interest as the expected correlation was not observed [CITE].
...
We estimate $G_{\textrm{\tiny asex}}$ to test whether greater asexual genetic diversity can be supported than a typically assumed one-to-many ratio [CITE].
...
Our study provides a practical for studying the Red Queen Hypothesis for sex for both theoreticians and empiricists.
...
In particular, even if the effect of Red Queen cannot be clearly detected, one cannot conclude that there is \emph{no effect} of parasite on maintenance of sex. \swp{Can I cite something here? Maybe Gelman?}
...
hence, it tells us more about asexual selection due to lack of parasitic risk \citep{lively2001trematode}.
...
Triangles and circles represent observed correlation from previous studies.
}
\label{fig:effect}
\end{figure}
...
Estimating biologically realistic parameters and testing theoretical models against data may provide deeper insight into underpinnings of maintenance of sexual reproduction.
