...
\swp{??}
\bmb{whether the data are consistent with more than a single asexual clone}
...
$G_{\textrm{\tiny asex}}$ & Number of asexual genotypes - 1 & $1 + \mathrm{BetaBinomial}(N=9, p=3/9, \theta=5)$ & Assumption\\
...
Overall, we predict that increasing the number of sites is a more effective way to increase power than increasing number of samples per site.
...
The similarity between the correlations predicted by the model and the correlations observed in nature may be interpreted as providing more confidence that the observed populations provide evidence for the Red Queen Hypothesis (\fref{effect}).
However, the strength of correlation does not provide any evidence for the strength of selection imposed on the host population. \swp{??} \bmb{not a direct measure of }
