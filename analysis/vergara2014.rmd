...
We tried replicating your work on the red queen dynamics in Vergara et al. 2014 (Infection dynamics in coexisting sexual and asexual host populations: support for the Red Queen hypothesis) and found some discrepancies between our analysis and the results/figures presented in the paper.

The data was downloaded from Dryad and imported into R. Following the methods section, we excluded males for our analysis. Observations that had "UNKOWN" ploidy were also excluded, and we obtained a data set with 1179 observations.
...
We fitted glm with a binomial error and logit link and performed ANOVA:
...
We get chi square values that are almost identical to the ones presented in the paper for all the interaction terms but not for first order terms. We get higher chi square values for all three effects.
...
For the prevalence of asexual snails across time, we get noticeably narrower standard errors compared to figure 1A in the paper. In figure 1B, we get a graph that is shifted upward by approximately 0.1. We tried taking the mean across sites first and averaging across those means but still were not able to match the results presented in the paper.
...
Prevalence of infection in 2002 at the West Point site has a noticeably larger standard error error than the one presented in the paper. We were able to match the figure in the paper when we switched from using the sample standard deviation to population standard deviation.
