...
\begin{itemize}
    \item \textbf{two modes of reproduction}: sexual and asexual
    \begin{itemize}
        \item an asexual organism can reproduce alone
        \item a sexual organism female requires a partner (male and female) to reproduce
    \end{itemize}
    \item sexual  population grows slower than asexual population because males cannot give birth
    \item if sexual reproduction is worse, why do so many organisms reproduce sexually?
\end{itemize}
\end{block}
...
\begin{itemize}
    ...
    \item many snails are infected by trematodes
    \item it is hypothesized that sexual reproduction is maintained in the snail population to \cite{vergara2014infection}
    \item sexual reproduction combines parent genes and can create new genes
