...
reslist <- vector('list', length(n.genotype_vec))

target.gen <- 1001:1100

for(n.genotype in n.genotype_vec) {
    ...
    reslist[[which(n.genotype==n.genotype_vec)]] <- res
}

names(reslist) <- n.genotype_vec

save("reslist", file="beta_sim.rda")
